...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.StreamHandler(sys.stdout),  # INFO and above to stdout
    ]
)
...
SUPABASE_URL = "https://bwgnfecyvfuburdiwbiv.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3Z25mZWN5dmZ1YnVyZGl3Yml2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc3MTcxNTAsImV4cCI6MjA5MzI5MzE1MH0.qRQTYD2dW0PWUNhozwZHEAVSJlUSUiRqCYmjvU7eTTs"

# Hugging Face API Configuration
HF_API_URL = "https://raoghulam-food-detection-api.hf.space/predict"
...
# Log config on startup (without exposing full keys)
logger.info("=" * 60)
...
logger.info(f"🔧 Environment: {'Production' if os.getenv('ENV') == 'prod' else 'Development'}")
logger.info("=" * 60)
...
async def serve_frontend():
    ...
    if os.path.exists("index.html"):
        api_logger.info(f"Serving index.html")
...
async def health_check():
    ...
    return {"status": "healthy", "api": "food-tray-recognition", "timestamp": datetime.now().isoformat()}
...
async def detect_food(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Detect food items using HF API
